"""Reward utilities for Evo steering."""
...
def get_reward_function(reward_name, images, prompts, metric_to_chase="overall_score"):
    if reward_name != "LLMGrader":
        print(
            "`metric_to_chase` will be ignored as it only applies to "
            "'LLMGrader' as the `reward_name`"
        )
    ...
    if reward_name == "HumanPreference":
...
def get_image_reward_model(name: str = "ImageReward-v1.0"):
    """Load and cache the ImageReward model for Evo steering."""
    global REWARDS_DICT

    if REWARDS_DICT["ImageReward"] is None:
        try:
            REWARDS_DICT["ImageReward"] = rm_load(name)
        except Exception as exc:
            raise RuntimeError(
                "Failed to initialize Evo ImageReward. This is often caused by a "
                "broken wandb/protobuf installation in the runtime. "
                "Try either: (1) install compatible wandb/protobuf, or "
                "(2) run with --guidance-reward-fn Clip-Score."
            ) from exc

    return REWARDS_DICT["ImageReward"]
...
def do_image_reward(*, images, prompts):
    """Compute ImageReward scores for a batch of PIL images and prompts."""
    model = get_image_reward_model()
    with torch.no_grad():
        return model.score_batched(prompts, images)
...
def do_llm_grading(*, images, prompts, metric_to_chase="overall_score"):
    ...
    if REWARDS_DICT["LLMGrader"] is None:
        try:
            ...
            raise RuntimeError("Failed to initialize Evo LLMGrader backend.") from exc
...
class CLIPScore(nn.Module):
    ...
    def score(self, prompt, pil_image, return_feature=False):
        ...
        rewards = torch.sum(
            torch.mul(txt_features, image_features),
            dim=1,
            keepdim=True,
        )
